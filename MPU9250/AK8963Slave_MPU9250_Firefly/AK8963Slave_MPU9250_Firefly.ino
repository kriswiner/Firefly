/* 03/14/2021 Copyright Tlera Corporation
 *  
 ...
 * 
 ...
 Sketch runs on the 3.3 V Firefly STM32WB55CG Breakout Board.
...
int  myLed  = LED_BUILTIN; // blue led
...
//    }
    ...
    if(alarmFlag) { // update RTC output (serial display) whenever the RTC alarm condition is achieved  
       alarmFlag = false;
    // Serial print and/or display at rate independent of RTC Alarm
//delt_t = millis() - count;
//    if (delt_t > 100) { // update at 100 ms intervals
    ...
    STM32WB.stop(); //put MCU into lowest power state while waiting for the next interrupt
