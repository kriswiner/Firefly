...
void MPU9250::magcalMPU9250(float * dest1, float * dest2) 
{
  ...
  Serial.print("Mag Calibration for: 0x"); Serial.println(MPU9250_ADDRESS, HEX); Serial.println("Wave device in a figure eight until done!");
  delay(4000);
...
// shoot for ~fifteen seconds of mag data
  ...
  for(ii = 0; ii < sample_count; ii++) {
    ...
    if(_Mmode == 0x06) delay(20);   // at 100 Hz ODR, new mag data is available every 10 ms
    readBytes(MPU9250_ADDRESS, EXT_SENS_DATA_00, 7, &rawData[0]);        // Read the x-, y-, and z-axis calibration values
